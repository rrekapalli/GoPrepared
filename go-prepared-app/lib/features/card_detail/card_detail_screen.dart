...
class CardDetailScreen extends ConsumerStatefulWidget {
  const CardDetailScreen({super.key, required this.cardId, this.journeyId});
...
class _CardDetailScreenState extends ConsumerState<CardDetailScreen> {
  ...
  String get _deckFallback {
    if (widget.journeyId != null) return '/journeys/${widget.journeyId}/deck';
    return '/journeys';
  }
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      final card = await repo.getCard(widget.cardId);
      CardModel? next;
  ...
  void _openNextCard() {
    ...
    if (next == null) {
      popOrGo(context, _deckFallback);
      return;
    }
    final journeyId = widget.journeyId;
    final path = journeyId != null
        ? '/cards/${next.id}?journeyId=$journeyId'
        : '/cards/${next.id}';
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        leading: BackButton(onPressed: () => popOrGo(context, _deckFallback)),
      ...
      body: ListView(
        ...
        children: [
          ...
          if (_nextCard != null) ...[
            const SizedBox(height: 16),
            FilledButton.icon(
              onPressed: _openNextCard,
              icon: const Icon(Icons.arrow_forward),
              label: Text('Next: ${_nextCard!.title}'),
              style: FilledButton.styleFrom(backgroundColor: AppColors.primary),
            ),
          ] else if (widget.journeyId != null) ...[
            const SizedBox(height: 16),
            OutlinedButton.icon(
              onPressed: () => popOrGo(context, _deckFallback),
              icon: const Icon(Icons.style_outlined),
              label: const Text('Back to prep deck'),
            ),
          ],
          const SizedBox(height: 12),
          OutlinedButton.icon(
